...
class DynamicCollegeCounselorBot:
    ...
    def _fetch_colleges_from_database(self) -> List[Dict]:
        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                logging.info(f"Fetched {len(colleges)} colleges from database")
                return colleges
    ...
    def _extract_fees_from_courses(self, courses_data) -> int:
        ...
        if not courses_data:
            return 50000  # Default fee if no data
        ...
        return int(total_fees / valid_courses) if valid_courses > 0 else 50000  # Default fee
    ...
    def _generate_highlights(self, row) -> List[str]:
        ...
        if scholarship and scholarship.lower() not in ["n/a", "not available", ""]:
    ...
    def _search_colleges_by_criteria(self, field_keywords=None, location=None, college_type=None) -> List[Dict]:
        ...
        try:
            ...
            if field_keywords:
                # Search in courses JSON and college type - make it more flexible
                field_conditions = []
                for i, keyword in enumerate(field_keywords):
                    param_name = f'field_keyword_{i}'
                    field_conditions.append(f"(LOWER(courses::text) LIKE :{param_name} OR LOWER(type) LIKE :{param_name} OR LOWER(college_name) LIKE :{param_name})")
            ...
            with self.engine.connect() as conn:
                ...
                for row in result:
                    ...
                    college_data = {
                        ...
                        "fees": self._extract_fees_from_courses(courses_data),
                        "highlights": self._generate_highlights(dict(zip([
                            'College_ID', 'College_Name', 'Name', 'Type', 'Affiliation', 'Location',
                            'Website', 'Contact', 'Email', 'Courses', 'Scholarship', 'Admission_Process'
                        ], row)))
                    }
                ...
                logging.info(f"Found {len(colleges)} colleges matching criteria")
                return colleges
    ...
    def _extract_student_information(self, user_message: str) -> Dict[str, Any]:
        ...
        tech_keywords = ["computer", "programming", "software", "coding", "tech", "it", "engineering"]
        ...
        medical_keywords = ["doctor", "medical", "medicine", "healthcare", "mbbs", "neet"]
        ...
        business_keywords = ["business", "management", "mba", "finance", "marketing", "commerce"]
        ...
        arts_keywords = ["arts", "literature", "history", "philosophy", "sociology", "psychology"]
        if any(word in message_lower for word in arts_keywords):
            if "Arts" not in self.student_profile.preferred_fields:
                self.student_profile.preferred_fields.append("Arts")
                self.student_profile.interests.append("Arts")
                updates["preferred_fields"] = self.student_profile.preferred_fields

        science_keywords = ["science", "physics", "chemistry", "biology", "mathematics", "research"]
        if any(word in message_lower for word in science_keywords):
            if "Science" not in self.student_profile.preferred_fields:
                self.student_profile.preferred_fields.append("Science")
                self.student_profile.interests.append("Science")
                updates["preferred_fields"] = self.student_profile.preferred_fields

        # === Location Preference ===
        for city in ["delhi", "mumbai", "bangalore", "pune", "hyderabad", "chennai", "kolkata", "indore", "kochi", "kerala", "karnataka", "tamil nadu", "maharashtra"]:
        ...
        # === Mark info collected - More lenient conditions ===
        if (self.student_profile.preferred_fields or 
            any(keyword in message_lower for keyword in ["college", "recommend", "suggest"]) or
    ...
    def _get_fallback_response(self, message):
        ...
        if any(word in message_lower for word in ["engineering", "iit", "jee", "computer science"]):
            return """Engineering offers excellent career prospects! Some top options include:
...
What specific aspect would you like to dive deeper into? I'm here to provide detailed insights to help you make informed decisions!"""
    ...
    def generate_personalized_recommendations(self, profile=None):
        ...
        try:
            ...
            for college in all_colleges:
                score = 10  # Base score for all colleges
                reasons = []
                
                # Check field alignment - make it more flexible
                if preferred_fields:
                    ...
                    if college_streams:
                        for pref in preferred_fields:
                            if any(pref.lower() in stream.lower() for stream in college_streams):
                                field_match = True
                                break
                    
                    # Check in courses data
                    if not field_match and college_courses:
                        for course in college_courses:
                            if isinstance(course, dict):
                                ...
                                if any(pref.lower() in course_category or pref.lower() in course_name for pref in preferred_fields):
                                    field_match = True
                                    break
                    
                    # Check college name and type
                    if not field_match:
                        for pref in preferred_fields:
                            if (pref.lower() in college_name or 
                                pref.lower() in college_type or
                                (pref.lower() == "computer science" and any(word in college_name for word in ["engineering", "technology", "technical"])) or
                                (pref.lower() == "medicine" and any(word in college_name for word in ["medical", "health"])) or
                                (pref.lower() == "business" and any(word in college_name for word in ["management", "commerce", "business"]))):
                                field_match = True
                                break
                    ...
                    if field_match:
                        ...
                        # Still give some score for general colleges
                        score += 10
                        reasons.append("General college with diverse programs")
                
                # Location preference
                if location_preference:
                    college_location = college.get('location', '').lower()
                    if location_preference.lower() in college_location:
                        score += 25
                        reasons.append("Preferred location")
                    elif any(word in college_location for word in location_preference.lower().split()):
                        score += 15
                        reasons.append("Near preferred location")
                
                # Quality indicators
                if college.get('affiliation') and college.get('affiliation') != "Not specified":
                    score += 15
                    
                if college.get('scholarship') and college.get('scholarship').lower() not in ["n/a", "not available", ""]:
                    score += 10
                ...
                # Add score based on highlights
                highlights = college.get('highlights', [])
                score += len(highlights) * 2
                
                # Always include colleges with minimum score
                if score >= 10:  # Very low threshold to ensure colleges are included
                    recommendations.append({
                        "name": college['name'] or "Unknown College",
                        "location": college['location'] or "Location not specified",
                        "fees": college.get('fees', 0),
                        "match_score": min(score, 100),
                        "match_reasons": reasons or ["College available in database"],
                        "type": college.get('type', 'General'),
                        "admission": college.get('admission', 'Contact college for admission details'),
                        "highlights": highlights[:3] if highlights else ["Quality education"],
                        "website": college.get('website'),
                        "contact": college.get('contact'),
                        "email": college.get('email'),
                        "scholarship": college.get('scholarship', 'Contact college for details'),
                        "affiliation": college.get('affiliation', 'Not specified'),
    ...
    def _create_default_recommendations(self, preferred_fields, location_preference):
        """Create default recommendations when database is empty or unavailable"""
        default_recommendations = [
            {
                "name": "Sample Engineering College",
                "location": location_preference or "India",
                "fees": 100000,
                "match_score": 70,
                "match_reasons": ["Engineering programs available"],
                "type": "Engineering",
                "admission": "JEE Main/State CET",
                "highlights": ["AICTE Approved", "Good Placement Record"],
                "website": None,
                "contact": "Contact college directly",
                "email": None,
                "scholarship": "Merit-based scholarships available",
                "affiliation": "University affiliated"
            },
            {
                "name": "Sample Arts & Science College", 
                "location": location_preference or "India",
                "fees": 50000,
                "match_score": 65,
                "match_reasons": ["Liberal arts programs"],
                "type": "Arts & Science",
                "admission": "Merit-based/Entrance exam",
                "highlights": ["UGC Recognized", "Research Opportunities"],
                "website": None,
                "contact": "Contact college directly", 
                "email": None,
                "scholarship": "Need-based aid available",
                "affiliation": "State university"
            }
        ]
        
        # Filter based on preferred fields if specified
        if preferred_fields:
            filtered_defaults = []
            for rec in default_recommendations:
                for field in preferred_fields:
                    if field.lower() in rec['name'].lower() or field.lower() in rec['type'].lower():
                        filtered_defaults.append(rec)
                        break
            return filtered_defaults if filtered_defaults else default_recommendations
